...
public class VRIKAnimatedLocomotion : MonoBehaviour {
	...
	void Update() {
		...
		if (dirHorMag > 0f && dirHorMag > maxDistanceFromHMD) transform.position += (dirHor / dirHorMag) * (dirHorMag - maxDistanceFromHMD);
		...
		if (isMoving) {
			// Stop moving
			if (distance < distanceS && velMag < velocityS) isMoving = false;
		} else {
			// Start moving
			if (distance >= distanceS || velMag >= velocityS) isMoving = true;
		}
		...
		if (turnTarget < 0.2f) turnTarget = 0f;
